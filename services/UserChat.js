const { Chat, UserChat } = require('@models');
...
const index = async data => {
    try {
        const result = await UserChat.findAndCountAll({
            where: getWhere(data),
...
const store = async data => {
    try {
        const result = await UserChat.create(data);
...
const update = async (data, id) => {
    try {
        await UserChat.update(data, { where: { id } });
        const result = UserChat.findByPk(id);
        return Object.assign(data, result.get({ plain: true }));
    } catch (error) {
        throw new Error(error.errors.map(e => e.message));
    }
...
const destroy = async data => {
    try {
        ...
        return await UserChat.destroy({ where: data });
    } catch (error) {
        throw new Error(error.errors.map(e => e.message));
    }
};
